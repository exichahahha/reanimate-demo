...
app.post("/api/stage1/process-notes", async (req, res) => {
  try {
    ...
    if (selectedModel === "intel-openvino-qwen3") {
      ...
      const promptText = `Return ONLY valid JSON.
...
{
  "summary": "A concise explanation of the topic in 80-120 words.",
  "keyConcepts": ["concept 1", "concept 2", "concept 3", "concept 4"],
  "suggestedTitles": ["title 1", "title 2", "title 3"],
  "difficultyLevel": "Beginner",
  "coreTakeaway": "One concise sentence."
}

IMPORTANT:
- summary MUST be complete.
- keyConcepts MUST contain exactly 4 items.
- suggestedTitles MUST contain exactly 3 items.
